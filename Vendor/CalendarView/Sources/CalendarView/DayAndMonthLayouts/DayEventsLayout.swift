...
struct EventsPlacement: Layout {
    ...
    func updateCache(_ cache: inout Cache, subviews: Subviews) {
        // A rescheduled event or zoom can change frames without changing count/width.
        cache = Cache()
    }
    ...
    func placeSubviews(in bounds: CGRect, proposal: ProposedViewSize, subviews: Subviews, cache: inout Cache) {
        let key = frameKey(width: bounds.width)
        if cache.key != key {
...
extension NSRange {
    ...
    init(_ startDate: Date, _ endDate: Date) {
        ...
        // The grid is labelled by wall-clock hours, not elapsed time since midnight (DST).
        ...
        let endMinutes = endDate >= nextDay ? 1440 : calendar.component(.hour, from: endDate) * 60 + calendar.component(.minute, from: endDate)
        let length = endMinutes > startMinutes ? endMinutes - startMinutes : max(1, Int(endDate.timeIntervalSince(startDate) / 60))
        self = NSRange(location: startMinutes, length: min(1440 - startMinutes, length))
    }
