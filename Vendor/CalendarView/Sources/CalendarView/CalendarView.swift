...
public struct CalendarView<DayEvent: View, MonthDay: View, WeekSwitcherDay: View, Header: View>: View {
    ...
    private var wideWeek: Bool { displayMode == .week && dynamicTypeSize.isAccessibilitySize }
    ...
    public var body: some View {
        VStack(spacing: 0) {
            headerBuilder(HeaderBuilderParams(
                ...
                tapGoToTodayClosure: {
                    fullscreenDate = displayMode == .week ? Date().startOfWeek(customizationParams.firstDayOfWeek).startOfDay : Date().startOfDay
                    anchorDate = Date()
                ...
                if wideWeek {
                    GeometryReader { geometry in
                        ScrollView(.horizontal) {
                            dayLayoutView.frame(width: max(geometry.size.width, 830), height: geometry.size.height)
                        }
                    }
                } else {
                    dayLayoutView
        ...
        .onChange(of: displayMode) {
            if displayMode == .week {
                fullscreenDate = fullscreenDate.startOfWeek(customizationParams.firstDayOfWeek).startOfDay
            }
            updateData()
        }
    ...
    private var dayLayoutView: some View {
        GeometryReader { g in
            InfiniteTabPageView(
                ...
                isCalendarScrolling: isCalendarScrolling || wideWeek,
                width: g.size.width,
                didEndAnimation: { direction in
                    fullscreenDate = fullscreenDate.adding(.day, value: direction * displayMode.rawValue)
                }
            ) { page in
                Group {
                    let date = Calendar.current.date(byAdding: .day, value: (page - currentPage) * displayMode.rawValue, to: fullscreenDate)?.startOfDay ?? fullscreenDate
                    DayLayout(
                        ...
                        onSelectDay: { day in fullscreenDate = day; displayMode = .day },
