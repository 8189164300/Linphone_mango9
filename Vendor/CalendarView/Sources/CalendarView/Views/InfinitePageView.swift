    ...
    private var dragGesture: some Gesture {
        DragGesture(minimumDistance: 100)
            .updating($translation) { value, state, _ in
                ...
                guard !isCalendarScrolling, abs(value.translation.width) > abs(value.translation.height) else { return }
                isDaySwiping = true
                state = translation
            }
            .onEnded { value in
                guard !isCalendarScrolling, abs(value.translation.width) > abs(value.translation.height) else {
                    offset = 0; isDaySwiping = false; return
                }
