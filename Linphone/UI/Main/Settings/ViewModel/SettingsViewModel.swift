...
class SettingsViewModel: ObservableObject {
	...
	func downloadAndApplyRemoteProvisioning() {
		...
		guard let provisioningURL = Mango9Configuration.verifiedProvisioningURL(from: remoteProvisioningUrl) else {
			Log.warn("\(SettingsViewModel.TAG) Rejected provisioning URI outside the Mango9 allowlist")
			return
		}
		
		CoreContext.shared.doOnCoreQueue { core in
			if core.provisioningUri != provisioningURL.absoluteString {
