						...
						VStack(spacing: 0) {
							...
							*/
							...
							.onTapGesture {
								...
								if callsIsOpen {
									Task {
										await mango9CallSettingsViewModel.load()
									}
								}
							}
							...
							if callsIsOpen {
								VStack(spacing: 0) {
									VStack(spacing: 30) {
										Mango9CallRoutingSettings(
											viewModel: mango9CallSettingsViewModel
										)
...
private struct Mango9CallRoutingSettings: View {
	@ObservedObject var viewModel: Mango9CallSettingsViewModel

	var body: some View {
		VStack(alignment: .leading, spacing: 18) {
			HStack {
				VStack(alignment: .leading, spacing: 3) {
					Text("Mango9 call routing")
						.default_text_style_700(styleSize: 16)
					if !viewModel.formattedLine.isEmpty {
						Text(viewModel.formattedLine)
				...
				.accessibilityLabel("Refresh call routing")
			}
		...
		VStack(alignment: .leading, spacing: 12) {
			HStack {
				VStack(alignment: .leading, spacing: 3) {
					...
					HStack(spacing: 4) {
						Image(
							systemName: viewModel.forwardingEnabled
								? "checkmark.circle.fill"
								: "minus.circle"
						)
							.font(.system(size: 11, weight: .semibold))
						Text(viewModel.forwardingEnabled ? "Enabled" : "Off")
							.default_text_style_600(styleSize: 12)
					}
					.foregroundStyle(
						viewModel.forwardingEnabled
							? Color.green
							: Color.grayMain2c500
					)
				}
			...
			TextField("Forward calls to", text: $viewModel.forwardingDestination)
			...
			Button {
				Task {
					await viewModel.saveForwarding()
				}
			} label: {
				HStack(spacing: 8) {
					if viewModel.isSavingForwarding {
						ProgressView()
							.progressViewStyle(CircularProgressViewStyle(tint: Color.white))
					}
					Text(viewModel.isSavingForwarding ? "Saving…" : "Save forwarding")
			...
			.buttonStyle(.plain)
			.disabled(
				viewModel.isSavingForwarding ||
				(viewModel.forwardingEnabled &&
				 viewModel.forwardingDestination.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty)
			)
			.opacity(
				(viewModel.forwardingEnabled &&
				 viewModel.forwardingDestination.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty)
