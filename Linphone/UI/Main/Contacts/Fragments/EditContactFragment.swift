...
struct EditContactFragment: View {
	...
	@FocusState var isEmailFocused: Int?
					...
					Image("caret-left")
						...
						.onTapGesture {
							if editContactViewModel.selectedEditFriend == nil
								...
								&& editContactViewModel.emails.first?.isEmpty ?? true
					...
					VStack(spacing: 0) {
						VStack(spacing: 0) {
							...
							VStack(alignment: .leading) {
								...
								ForEach(editContactViewModel.emails.indices, id: \.self) { index in
									HStack(alignment: .center) {
										TextField("contact_email", text: $editContactViewModel.emails[index])
											.default_text_style(styleSize: 15)
											...
											.overlay(
												RoundedRectangle(cornerRadius: 60)
													.inset(by: 0.5)
													.stroke(isEmailFocused == index ? Color.orangeMain500 : Color.gray200, lineWidth: 1)
											)
											.focused($isEmailFocused, equals: index)
											.onChange(of: editContactViewModel.emails[index]) { newValue in
												if !newValue.isEmpty && index == editContactViewModel.emails.count - 1 {
													withAnimation {
														editContactViewModel.emails.append("")
													}
												}
											}

										Button(action: {
											guard editContactViewModel.emails.indices.contains(index) else { return }
											editContactViewModel.emails.remove(at: index)
										}) {
											Image("x")
												.renderingMode(.template)
												.resizable()
												.foregroundStyle(
													editContactViewModel.emails[index].isEmpty && index == editContactViewModel.emails.count - 1
													? Color.gray100
													: Color.grayMain2c600
												)
												.frame(width: 25, height: 25)
										...
										.disabled(editContactViewModel.emails[index].isEmpty && index == editContactViewModel.emails.count - 1)
									}
									.zIndex(isEmailFocused == index ? 1 : 0)
	...
	func addOrEditFriend() {
		CoreContext.shared.doOnCoreQueue { core in
			let newContact = Contact(
				...
				emails: editContactViewModel.emails,
				imageData: ""
			)
