...
class CoreContext: ObservableObject {
	...
	private func handleConfigurationChanged(status: ConfiguringState) {
		...
		if status == .Successful {
			enforceMango9AccountRouting(core: self.mCore)
	...
	func initialiseCore() throws {
		...
		coreQueue.async {
			...
			let userAgent = "Mango9iOS/\(appGitTag) (\(UIDevice.current.localizedModel.replacingOccurrences(of: "'", with: "")))"
			...
			self.mCoreDelegate = CoreDelegateStub(onGlobalStateChanged: { (core: Core, state: GlobalState, _: String) in
				...
				if method == .Bearer {
					if let server = authInfo.authorizationServer, !server.isEmpty {
						guard Mango9Configuration.verifiedProvisioningURL(from: server) != nil else {
							Log.warn("Rejected a Bearer authentication server outside the Mango9 allowlist")
							return
						}
						Log.info("Authentication requested method is Bearer, starting Mango9 Single Sign On")
	...
	private func enforceMango9AccountRouting(core: Core) {
		for account in core.accountList {
			...
			do {
				let serverAddress = try Factory.Instance.createAddress(addr: Mango9Configuration.sipProxyURI)
