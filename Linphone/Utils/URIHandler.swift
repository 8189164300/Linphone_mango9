...
class URIHandler {
	...
	private static let callSchemes = ["sip", "tel", "callto"]
	private static let secureCallSchemes = ["sips"]
	private static let configurationSchemes = ["mango9-config"]
	private static let sharedExtensionSchemes = ["mango9-message"]
	private static let mentionSchemes = ["mango9-mention"]
	...
	static func handleURL(url: URL) {
		if let scheme = url.scheme {
			Log.info("[URIHandler] handling URL with scheme \(scheme)")
		...
		if autoRemoteProvisioningOnConfigUriHandler() {
			CoreContext.shared.doOnCoreQueue { core in
				do {
					...
					guard let provisioningURL = Mango9Configuration.verifiedProvisioningURL(from: urlString) else {
						Log.warn("[URIHandler] rejected a provisioning URL outside the Mango9 allowlist")
						toast("Failed_uri_handler_bad_config_address")
						return
					}
					
					Log.info("[URIHandler] applying Mango9 provisioning configuration")
					core.config?.setString(section: "misc", key: "config-uri", value: provisioningURL.absoluteString)
					try core.setProvisioninguri(newValue: provisioningURL.absoluteString)
					core.stop()
					try core.start()
				} catch {
					Log.error("[URIHandler] unable to apply Mango9 provisioning configuration: \(error)")
					toast("Failed_uri_handler_bad_config_address")
				}
			}
		} else {
