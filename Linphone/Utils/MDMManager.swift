...
class MDMManager {
	...
	func loadXMLConfigFromMdm(config: Config) {
		...
		do {
			...
			Log.error("[MDMManager] Failed loading xmlConfig: error = \(error)")
		}
	}
	...
	func applyMdmConfigToCore(core: Core) {
		...
		if let configUri = mdmConfig["configUri"] as? String {
			do {
				guard let provisioningURL = Mango9Configuration.verifiedProvisioningURL(from: configUri) else {
					Log.warn("[MDMManager] Rejected configUri outside the Mango9 allowlist")
					return
				}
				if provisioningURL.absoluteString != currentProvisioningUri {
					try core.setProvisioninguri(newValue: provisioningURL.absoluteString)
					Log.info("[MDMManager] Mango9 configUri applied")
				}
			} catch let error {
				Log.error("[MDMManager] Failed setting provisioning URI: error = \(error)")
			}
		}
