...
final class ContactsManager: ObservableObject {
	...
	static let emailVCardProperty = "X-MANGO9-EMAIL"
	...
	private let mango9TeamFriendListName = "Mango9 team"
	
	var friendList: FriendList?
	var linphoneFriendList: FriendList?
    var tempRemoteFriendList: FriendList?
	private var mango9TeamFriendList: FriendList?
	
	@Published var lastSearch: [SearchResult] = []
	@Published var lastSearchSuggestions: [SearchResult] = []
	@Published private(set) var mango9ChatTargetsByAddress: [String: Mango9ChatTarget] = [:]
	...
	func fetchContacts() {
		self.coreContext.doOnCoreQueue { core in
			if core.globalState == GlobalState.Shutdown || core.globalState == GlobalState.Off {
				...
				do {
					self.mango9TeamFriendList = try core.getFriendListByName(name: self.mango9TeamFriendListName) ?? core.createFriendList()
				} catch let error {
					print("\(#function) - Failed to enumerate Mango9 team contacts: \(error)")
				}

				if let mango9TeamFriendList = self.mango9TeamFriendList,
				   mango9TeamFriendList.displayName == nil || mango9TeamFriendList.displayName!.isEmpty {
					mango9TeamFriendList.databaseStorageEnabled = true
					mango9TeamFriendList.displayName = self.mango9TeamFriendListName
					core.addFriendList(list: mango9TeamFriendList)
				}
	...
	func syncMango9Team(_ members: [Mango9TeamMember]) {
		var chatTargets: [String: Mango9ChatTarget] = [:]
		for member in members {
			guard let sipUri = member.sipUri?.lowercased(), !sipUri.isEmpty else {
				continue
			}
			chatTargets[sipUri] = Mango9ChatTarget(
				userId: member.userId,
				name: member.displayName
			)
		}
		DispatchQueue.main.async {
			self.mango9ChatTargetsByAddress = chatTargets
		}

		self.coreContext.doOnCoreQueue { core in
			do {
				let list = try core.getFriendListByName(name: self.mango9TeamFriendListName) ?? core.createFriendList()
				if list.displayName == nil || list.displayName!.isEmpty {
					list.databaseStorageEnabled = true
					list.displayName = self.mango9TeamFriendListName
					core.addFriendList(list: list)
				}
				self.mango9TeamFriendList = list

				let memberIds = Set(members.map { "mango9-user-\($0.userId)" })
				for friend in list.friends where !memberIds.contains(friend.nativeUri ?? "") {
					_ = list.removeFriend(linphoneFriend: friend)
				}

				for member in members {
					guard let sipUri = member.sipUri,
						  let address = try? Factory.Instance.createAddress(addr: sipUri) else {
						continue
					}

					let nativeUri = "mango9-user-\(member.userId)"
					let existing = list.friends.first { $0.nativeUri == nativeUri }
					let friend = try existing ?? core.createFriend()
					friend.edit()
					friend.nativeUri = nativeUri
					try friend.setName(newValue: member.displayName)

					if friend.vcard == nil {
						_ = try friend.createVcard(name: member.displayName)
					}
					friend.vcard?.givenName = member.displayName
					friend.vcard?.familyName = ""
					friend.vcard?.removeExtentedPropertiesByName(name: Self.emailVCardProperty)
					if !member.email.isEmpty {
						friend.vcard?.addExtendedProperty(name: Self.emailVCardProperty, value: member.email)
					}

					friend.addresses.forEach { friend.removeAddress(address: $0) }
					friend.addAddress(address: address)
					friend.phoneNumbersWithLabel.forEach {
						friend.removePhoneNumberWithLabel(phoneNumber: $0)
					}
					if !member.mobile.isEmpty,
					   let mobile = try? Factory.Instance.createFriendPhoneNumber(
						phoneNumber: member.mobile,
						label: "mobile"
					   ) {
						friend.addPhoneNumberWithLabel(phoneNumber: mobile)
					}
					friend.organization = "Mango9"
					friend.jobTitle = member.role.capitalized
				...
				Log.error("\(Self.TAG) Failed to synchronize Mango9 team contacts: \(error)")
			}
		}
	}

	func mango9ChatTarget(for address: Address) -> Mango9ChatTarget? {
		mango9ChatTargetsByAddress[address.asStringUriOnly().lowercased()]
	}

	func mango9ChatTarget(forNativeUri nativeUri: String) -> Mango9ChatTarget? {
		guard nativeUri.hasPrefix("mango9-user-"),
			  let userId = Int(nativeUri.dropFirst("mango9-user-".count)) else {
			return nil
		}
		return mango9ChatTargetsByAddress.values.first { $0.userId == userId }
	}

	func suggestionDisplayName(for result: SearchResult) -> String {
		if let address = result.address,
		   let mango9Target = mango9ChatTarget(for: address) {
			let name = mango9Target.name.trimmingCharacters(in: .whitespacesAndNewlines)
			if !name.isEmpty {
				return name
			}
		}

		if let friendName = result.friend?.name?
			.trimmingCharacters(in: .whitespacesAndNewlines),
		   !friendName.isEmpty {
			return friendName
		}

		if let displayName = result.address?.displayName?
			.trimmingCharacters(in: .whitespacesAndNewlines),
		   !displayName.isEmpty {
			return displayName
		}

		if let username = result.address?.username?
			.trimmingCharacters(in: .whitespacesAndNewlines),
		   !username.isEmpty {
			return username
		}

		let uri = result.address?.asStringUriOnly() ?? ""
		return uri.lowercased().hasPrefix("sip:")
			? String(uri.dropFirst(4))
			: uri
	}
	...
	func saveFriend(result: String, contact: Contact, existingFriend: Friend?, completion: @escaping (Friend?) -> Void) {
		self.coreContext.doOnCoreQueue { core in
			do {
				...
				if friend.vcard == nil {
					_ = try friend.createVcard(name: contact.firstName + " " + contact.lastName)
				}
				...
				if let vcard = friend.vcard {
					...
					vcard.removeExtentedPropertiesByName(name: Self.emailVCardProperty)
					for email in contact.emails where !email.isEmpty {
						vcard.addExtendedProperty(name: Self.emailVCardProperty, value: email)
					}
				}
						...
						friendList.friends.forEach { friend in
							...
							let newContact = Contact(
								...
								jobTitle: friend.jobTitle ?? "",
									...
									emails: friend.vcard?.getExtendedPropertiesValuesByName(name: Self.emailVCardProperty) ?? [],
									imageData: ""
								)
